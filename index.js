...
function updateLock(file, options) {
    ...
    lock.updateTimeout = setTimeout(() => {
      ...
      lock.updateTimeout = null;
        options.fs.stat(getLockFile(file), (err, stats) => {
          if (err) {
            if (err.code === 'ENOENT') {
          ...
          // First check is if lock file has gone stale
          // Second check is if another process has started updating the same lock file
          if (mtime <= now - options.stale || mtime > now - lock.updateDelay) {
              return compromisedLock(file, lock, Object.assign(new Error(lock.updateError || 'Lock has either gone stale or \
              // been taken over by another process'), { code: 'ECOMPROMISED' }));
            }
          options.fs.utimes(getLockFile(file), now/1000, now/1000, (err) => {
    ...
    canonicalPath(file, options, (err, file) => {
        ...
        operation.attempt(() => {
            acquireLock(file, options, (err) => {
                ...
                // if lock is acquired but we should already own it then lock was been manually removed
                if (locks[file]) {
                  return compromisedLock(file, locks[file], Object.assign(new Error('Lock has been manually removed or compromised'), { code: 'ECOMPROMISED' }));
                }
