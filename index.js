...
function acquireLock(file, options, callback) {
  ...
  options.fs.mkdir(getLockFile(file), function (err) {
      ...
      options.fs.stat(getLockFile(file), function (err, stat) {
          ...
          let notStale = stat.mtime.getTime() >= Date.now() - options.stale;
          if (notStale) {
            return callback(errcode('Lock file is already being hold', 'ELOCKED', { file: file }));
          }
...
function updateLock(file, options) {
  ...
  lock.updateTimeout = setTimeout(function () {
      ...
      options.fs.utimes(getLockFile(file), mtime, mtime, function (err) {
          ...
          // This situation is only possible if the main thread is paused (either through debugger or putting comp to sleep)
          // In this case another instance (other atom window) might have taken over the lock..
          // Release this local lock.
          if (lock.lastUpdate <= Date.now() - options.stale) {
            lock.released = true;
            clearTimeout(lock.updateTimeout);
            return options.callback(errcode(err, 'ECOMPROMISED'));
          }
          ...
          if (err) {
            if (err.code === 'ENOENT') {
              lock.released = true;
              return options.callback(errcode(err, 'ENOENT'));
            }

            lock.updateError = err;
            lock.updateDelay = options.stale / 4;
